    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="form-group col-md-3">
                    <br/>
                    <button class="btn btn-primary" type="submit" @click="makeDeposit">Sacar</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods:{
        makeDeposit(){
            axios.post(`${config.apiURL}/withdraw`, {account_id: this.currentAccount, amount: this.amount})
            .then(() => {
                this.$emit('displayMessage', {error: false, msg: 'Saque realizado.'})
            })
            .catch((error) => {
